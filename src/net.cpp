...
void AdvertiseLocal(CNode *pnode, uint64_t nLocalServices)
{
    ...
    {
        CAddress addrLocal = GetLocalAddress(&pnode->addr, nLocalServices);
...
bool StartNode(CConnman& connman, boost::thread_group& threadGroup, CScheduler& scheduler, uint64_t nLocalServices, int nMaxConnectionsIn, int nMaxOutboundIn, int nBestHeightIn, CClientUIInterface* interfaceIn, std::string& strNodeError)
{
    ...
    bool ret = connman.Start(threadGroup, scheduler, nLocalServices, nMaxConnectionsIn, nMaxOutboundIn, nBestHeightIn, interfaceIn, strNodeError);
