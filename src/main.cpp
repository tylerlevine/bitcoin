    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (control) {
                ...
                // Must use std::ref to make sure destructor only gets called once.
                result = CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, txdata[i], std::ref(emplacer));
            } else
                result = CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, txdata[i]);
            if (!result)
