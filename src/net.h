...
public:
    ...
    CConnman();
    ~CConnman();
    bool Start(boost::thread_group& threadGroup, CScheduler& scheduler, uint64_t nLocalServices, int nMaxConnectionsIn, int nMaxOutboundIn, int nBestHeightIn, CClientUIInterface* interfaceIn, std::string& strNodeError);
    void Stop();
...
void AdvertiseLocal(CNode *pnode, uint64_t nLocalServices);
...
public:
    ...
    CNode(NodeId id, uint64_t nLocalServicesIn, int nBestHeightIn, SOCKET hSocketIn, const CAddress &addrIn, const std::string &addrNameIn = "", bool fInboundIn = false);
    ~CNode();
