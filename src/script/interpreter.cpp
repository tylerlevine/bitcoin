...
bool EvalTapScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, ScriptExecutionData execdata)
{
    static const CScriptNum bnZero(0);
    static const CScriptNum bnOne(1);
    // static const CScriptNum bnFalse(0);
    ...
    try
    {
        while (pc < pend)
        {
            ...
            // Read instruction
            //
            if (!script.GetTapOp(pc, opcode, vchPushValue))
                return set_error(serror, SCRIPT_ERR_BAD_OPCODE);
            if (vchPushValue.size() > MAX_SCRIPT_ELEMENT_SIZE)
                return set_error(serror, SCRIPT_ERR_PUSH_SIZE);
            ...
            switch (opcode)
            {
                ...
                {
                    // Allows disabling verification to check other conditions
                    if (flags & SCRIPT_VERIFY_BAG_SECURED) {
                        // Check that the Bag hash matches the immediate value
...
bool GenericTransactionSignatureChecker<T>::CheckBagSecured(const std::vector<unsigned char>& hash) const
{
    ...
    if (txdata && txdata->ready) {
        hashOutputs = &txdata->m_bag_hash;
    } else {
        assert(txTo != nullptr);
        tmp = GetSecuredBagHash(*txTo);
    }
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (witversion == 0) {
        ...
        flags |= SCRIPT_VERIFY_BAG_SECURED;
        ...
        if ((control[0] & 0xfe) == 0xc0) {
            ...
            while (pc < scriptPubKey.end()) {
                opcodetype opcode;
                if (!scriptPubKey.GetTapOp(pc, opcode)) {
        ...
        if (!EvalTapScript(stack, scriptPubKey, flags, checker, sigversion, serror, execdata)) {
            return false;
        }
