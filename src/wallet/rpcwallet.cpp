...
static UniValue sendmanycompacted(const JSONRPCRequest& request)
{
    ...
    while (vecSend.size() > 1) {
        ...
        const uint256 hash = GetSecuredBagHash(tx);
        ...
        vecSend.emplace_back(amount, CScript() << OP_SECURETHEBAG << h_buff);
    }
    ...
    std::string fail_reason;
    {
        ...
        CValidationState state;
        if (!wallet.CommitTransaction(tx, mapValue, {} /* orderForm */, state)) {
            fail_reason = strprintf("Transaction commit failed:: %s", FormatStateMessage(state));
            throw JSONRPCError(RPC_WALLET_ERROR, fail_reason);
        }
    }

    // Fill out the templates for all the children transactions and Commit
    std::vector<std::pair<COutPoint, uint256>> stack {{{tx->GetHash(), root_idx}, {}}};
    std::memmove(stack.back().second.begin(), tx->vout[root_idx].scriptPubKey.data()+2, 32);
    ...
    while (!stack.empty()) {
        ...
        for (auto& out : tx->vout) {
            if (out.scriptPubKey.IsBasicSecureTheBag()) {
                stack.emplace_back(COutPoint{txid, i}, uint256{});
                std::memmove(stack.back().second.begin(), out.scriptPubKey.data()+2, 32);
            }
            ++i;
        }
    }
