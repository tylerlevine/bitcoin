...
static bool IsSecureTheBagEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
    return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SECURETHEBAG, versionbitscache) == ThresholdState::ACTIVE);
}
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (IsSecureTheBagEnabled(pindex->pprev, consensusparams)) {
        flags |= SCRIPT_VERIFY_SECURETHEBAG;
    }
