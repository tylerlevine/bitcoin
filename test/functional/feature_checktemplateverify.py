...
def template_hash_for_outputs(outputs):
    c = CTransaction()
    c.nVersion = 2
    c.vin = [CTxIn()]
    c.vout = outputs
    return c.get_standard_template_hash()
def random_p2sh():
...
def random_real_outputs_and_script(n):
    ...
    script  = CScript(bytes([0x20]) + template_hash_for_outputs(outputs) + bytes([OP_CHECKTEMPLATEVERIFY]))
...
class CheckTemplateVerifyTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        check_template_verify_tx.vin = [CTxIn(segwit_stb_outpoint)]
        ...
        check_template_verify_two_inputs.vin += [CTxIn(anyone_can_spend_outpoint, b"\x01\x51")]
